...
const Login = () => {
  ...
  const validateFields = () => {
    ...
    requiredFields.forEach((field) => {
      if (!eval(field)) {
        // Use eval to dynamically access state variables
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (otpOption === "sms" || otpOption === "email") {
        await axios.post(
          `http://localhost:8001/login/send-otp/${otpOption}/${username}`,
          {
            password,
          }
        );
        ...
        Swal.fire(
          ...
          `OTP sent successfully to ${
            otpOption === "sms" ? "your phone" : "your email"
          }`,
          "success"
        );
      } else {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:8001/login/verify-otp/${username}`,
        {
          otp,
        }
      );
      ...
      if (response.status === 200) {
        ...
        Swal.fire(
          "Success",
          "OTP and password verified successfully",
          "success"
        );
        ...
        if(localStorage.getItem("isSubProfileSet") === "true") {
